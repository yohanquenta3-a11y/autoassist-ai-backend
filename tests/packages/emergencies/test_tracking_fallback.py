...
async def test_get_latest_tracking_real_priority():
    ...
    mock_incident.estado_incidente = "EN_CAMINO"
...
@pytest.mark.asyncio
async def test_get_latest_tracking_not_en_camino():
    # Setup
    mock_db = AsyncMock()
    mock_user = MagicMock()
    incident_id = uuid.uuid4()
    ...
    # Mock incident in state EN_ATENCION
    mock_incident = MagicMock(spec=Incidente)
    mock_incident.id_incidente = incident_id
    mock_incident.estado_incidente = "EN_ATENCION"
    mock_incident.ubicacion_emergencia = MagicMock() # Non-null, so it would normally query Directions API
    ...
    with patch("app.packages.emergencies.presentation.routers.IncidentRepository") as mock_repo_class:
        ...
        # Assertions: route and eta must be None because state is EN_ATENCION
        assert result["latitud"] == -17.78
        assert result["longitud"] == -63.18
        assert result["eta_minutos"] is None
        assert result["polyline_ruta"] is None
        assert result["has_tracking"] is True
